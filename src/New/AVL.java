...
//Если нет ближайщего большего позначению,чем заданый ключ, то возвращает null
    ...
    private Node add (Node node,Key key, Value value, Integer indexNumberInETour, Node father){
        if (node == null){
            ...
            newnode.last = indexNumberInETour;
            return newnode;
        }

        int compareResult = key.compareTo(node.key);
        if (compareResult > 0){node.right = add(node.right, key, value, indexNumberInETour, node); node.h = height(node.left, node.right) + 1;}
        else if(compareResult < 0){node.left = add(node.left, key, value, indexNumberInETour, node); node.h = height(node.left, node.right) + 1;}
        else{
            node.value = value;
            node.last = indexNumberInETour;
        }
